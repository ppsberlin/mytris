...
;	sta POKEY2,x
    sta SDWPOK1,x 			;/// edit by pps
    sta SDWPOK0,x 			;// edit by pps
    bcs     @DontSetBuffer
@DontSetPokey    
@SMSet_LZSSBuf    
...
;	sta POKEY2+0
;    sta    POKEY+0
    sta SDWPOK1+0 			;// edit by VinsCool: write to POKEY buffer for VUPlayer's timing and VUMeter display
    sta SDWPOK0+0 			;// edit by pps
    //---
UpdatePokeyRegisters1
.REPT 8 #
    lda    LZSSBuffers+(#+1)*256,x    
SMSet_WritePokey:1
    sta SDWPOK1+(#+1) 			;// edit by VinsCool: write to POKEY buffer for VUPlayer's timing and VUMeter display
    sta SDWPOK0+(#+1) 			;// edit by pps
;    sta	POKEY2+(#+1)
;    sta    POKEY+(#+1)
.ENDR  
...
;--- for mono replay
	.proc LZSSUpdatePokeyRegisters_mono
    //--- get offset into decoded buffers where lasts bytes were written to
    ldx     LZS.DLIDstBufOffset
    inc     LZS.DLIDstBufOffset
    ...
    lda    LZSSBuffers+0*256,x      
;    sta    POKEY+0
    sta SDWPOK0+0 			;// edit by VinsCool: write to POKEY buffer for VUPlayer's timing and VUMeter display
    //---
UpdatePokeyRegisters1
.REPT 8 #
    lda    LZSSBuffers+(#+1)*256,x    
